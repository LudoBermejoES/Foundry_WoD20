...
// La REGLA, no un umbral calibrado a la forma de hoy: por cada sitio donde la tira pinta el
// icono REAL tiene que haber un sitio que pinte el HUECO. Asi la fila declara siempre el mismo
// contenido fijo y los circulos y el coste no se desplazan. Contar «al menos N sitios» ataba
// esta comprobacion al numero de ramas que tuviera la plantilla ese dia, y se rompio en cuanto
// la pestaña simplifico la condicion de dos ramas a una.
const realSites = iconSites - slotSites;
check(`F1 cada sitio que pinta el icono real tiene su sitio de hueco ` +
	`(${realSites} real(es), ${slotSites} hueco(s))`,
	realSites >= 1 && slotSites >= 1 && realSites === slotSites);
