export default class IconHelper {
    ...
    static GetIcon(icon, type, number = undefined) {        
        ...
        switch (icon) {
            ...
            case "note":
                ...
                break;
            case "connections":
                data = this._getConnections(type);
                break;
            case "shapechange":
    ...
    static GetIconlist(race) {
        let iconlist = {
			...
			connections: this.GetIcon("connections", race),
    ...
    /**
     * add-allies-contacts-tab — the "Aliados y contactos" rail icon: two overlapping person
     * silhouettes (a generic "group" pictogram, not sourced from any particular icon set — see
     * README.md's game-icons.net attribution for the rest of this file's glyphs). Same shape as
     * every other icon here: one glyph, `_getTypeColor(type)` supplies the background so it recolors
     * per splat automatically, no per-splat branch.
     */
    static _getConnections(type, height = 30, width = 30) {
        let stroke = "#000";

        return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" style="border-radius: 3px; height: ${height}px; width: ${width}px;"><path d="M0 0h512v512H0z" fill="${this._getTypeColor(type)}" fill-opacity="1"></path><g class="" transform="translate(0,0)" style=""><rect x="240" y="270" width="220" height="150" rx="60" fill="#fff" fill-opacity="1"></rect><circle cx="350" cy="190" r="50" fill="#fff" fill-opacity="1"></circle><rect x="60" y="240" width="260" height="180" rx="70" fill="#fff" fill-opacity="1"></rect><circle cx="190" cy="150" r="58" fill="#fff" fill-opacity="1"></circle></g></svg>`;
    }
