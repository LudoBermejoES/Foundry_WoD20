...
function syntheticChantry(rosters) {
	...
	const actor = {
		...
		flags: {},
		getFlag(scope, key) {
			return `${scope}.${key}`.split(".").reduce((n, k) => (n == null ? undefined : n[k]), actor.flags);
		},
		async unsetFlag(scope, key) {
			const parts = `${scope}.${key}`.split(".");
			let node = actor.flags;
			while (parts.length > 1) { node = node?.[parts.shift()]; if (!node) return; }
			delete node[parts[0]];
		},
...
async function migrateOne(actor) {
	...
	// La guarda del segundo intento, igual que en `migrations.js`: un fallo de verificacion
	// deja el mapa sin vaciar, asi que sin esto el predicado seguiria casando y la pasada
	// siguiente crearia OTRA copia. Se escribe ANTES de tocar nada.
	if (actor.getFlag(MIGRATION_FLAG_SCOPE, ATTEMPT_FLAG_KEY)) return { touched: false, skipped: true };
...
await test("el verificador CAZA una entrada perdida, un nombre cambiado, un punto cambiado y una de más", () => {
	...
	// La REGLA es «un valor de puntos cambiado tiene que fallar», no «tiene que decirlo con
	// esta frase». Desde que la comparacion es por MULTICONJUNTO (el orden de `actor.items` no
	// esta garantizado y comparar por indice reportaba cambios de nombre fantasma), un punto
	// cambiado se describe como una entrada que falta con su valor viejo y otra que sobra con
	// el nuevo. Sigue siendo un fallo, y eso es lo que se afirma.
	assert.equal(repointed.ok, false);
	assert.match(repointed.mismatches.join(" "), /falta "Tzippi Jessel" \(1 punto\/s\)/);
	assert.match(repointed.mismatches.join(" "), /sobra "Tzippi Jessel" \(3 punto\/s\)/);
...
// ─────────────────────────────────────────────────────────────────────────────────────
// EL TEST QUE FALTABA, y el que habria evitado el incidente. El anterior probaba la segunda
// pasada SOLO despues de un exito, que es el camino que no falla. El camino que falla es
// este: la verificacion no cuadra -> el mapa NO se vacia (correcto) -> ¿que hace el arranque
// siguiente? Antes: crear otra copia de todo. Tres arranques dieron tres copias de cada
// Custos en una Capilla real de produccion.
await test("con la verificacion FALLANDO, la segunda pasada no crea NI UN Item mas", async () => {
	const actor = syntheticChantry(MEKARCHITEK());

	// Se fuerza el fallo por el unico camino que no toca el codigo bajo prueba: la creacion
	// devuelve un Item de menos, asi que el conteo no puede cuadrar.
	const realCreate = actor.createEmbeddedDocuments.bind(actor);
	actor.createEmbeddedDocuments = async (type, data) => realCreate(type, data.slice(0, -1));

	const primera = await migrateOne(actor);
	assert.equal(primera.result.ok, false, "el montaje debe hacer fallar la verificacion");
	const tras1 = actor.items.length;
	assert.ok(tras1 > 0);
	assert.ok(Object.keys(actor.system.traitRosters).length > 0,
		"con la verificacion en rojo el mapa se conserva: el dato queda en los dos portadores");

	const segunda = await migrateOne(actor);
	assert.equal(actor.items.length, tras1,
		`la segunda pasada creo ${actor.items.length - tras1} Item(s) de mas: esto es exactamente `
		+ "el defecto que duplicaba el censo en cada arranque del mundo");
	assert.ok(segunda.skipped, "y debe decir que se la salto, no repetirla en silencio");

	// Y una tercera, porque el incidente real fueron TRES arranques.
	await migrateOne(actor);
	assert.equal(actor.items.length, tras1);
});

// ─────────────────────────────────────────────────────────────────────────────────────
// LA REGRESION DIRECTA DEL INCIDENTE. `before` sale del mapa, con su orden de insercion;
// `after` sale de `actor.items`, cuyo orden Foundry NO garantiza. La version anterior
// comparaba `was[i]` contra `now[i]` y por tanto fallaba con las MISMAS entradas en otro
// orden, reportando un cambio de nombre fantasma. Medido en vivo: la misma migracion
// devolvio una vez «Jim, Raffela, John, Tzippi» y otra «Raffela, Jim, John, Tzippi».
//
// El orden no es informacion: la hoja ordena cada grupo por nombre con `localeCompare`
// antes de pintarlo. Lo que hay que preservar es el CONJUNTO con sus puntos.
await test("el mismo censo en OTRO ORDEN cuadra: se compara el conjunto, no el indice", () => {
	const mapa = MEKARCHITEK();
	const before = snapshotFromRosters(mapa);

	const item = (n, p = 1) => ({
		type: "Feature",
		name: n,
		system: { type: "wod.types.connection", relation: "allies", points: p },
	});
	const nombres = (mapa.allies ?? []).map((e) => e.name);
	assert.ok(nombres.length >= 3, "el fixture necesita al menos tres entradas para poder permutar");

	// Cada rotacion del mismo conjunto tiene que cuadrar. Un fixture con una sola permutacion
	// no distingue «compara conjuntos» de «compara indices y tuvo suerte».
	for (let r = 0; r < nombres.length; r++) {
		const rotado = [...nombres.slice(r), ...nombres.slice(0, r)];
		const after = snapshotFromItems(rotado.map((n) => item(n)));
		const res = verifyCensusMigration(before, after);
		assert.ok(res.ok,
			`la rotacion ${r} (${rotado.join(", ")}) deberia cuadrar y no cuadra: `
			+ res.mismatches.join("; "));
	}

	// Y el orden inverso, que es el caso extremo.
	const inverso = snapshotFromItems([...nombres].reverse().map((n) => item(n)));
	assert.ok(verifyCensusMigration(before, inverso).ok);

	// Contraprueba: cambiar UN nombre sigue fallando, para que esto no sea un test que
	// aprueba cualquier cosa.
	const trucado = snapshotFromItems(
		[...nombres.slice(0, -1), "Alguien Que No Estaba"].map((n) => item(n)));
	assert.equal(verifyCensusMigration(before, trucado).ok, false);
});
...
await test("el paseo real de migrations.js hace las cuatro escrituras en el orden que las hace migrateOne", () => {
	...
	// ESTE GATE AFIRMABA EL DEFECTO. Exigia que el paseo NO leyera bandera alguna, sobre la
	// premisa de que el predicado («un mapa no vacio») no puede casar despues de migrar. Es
	// falso cuando la verificacion falla: entonces el mapa NO se vacia —correctamente— el
	// predicado sigue casando, y el arranque siguiente DUPLICA. Paso en produccion: tres
	// arranques, tres copias de cada Custos. La regla nueva es la contraria.
	assert.match(src, /getFlag\([^)]*ATTEMPT_FLAG_KEY/,
	...
	assert.ok(iSet !== -1 && iCreate !== -1 && iSet < iCreate,
		"la marca de intento se escribe ANTES de crear Items: si la creacion se queda a medias, "
		+ "el arranque siguiente tampoco debe reintentar a ciegas");
});
